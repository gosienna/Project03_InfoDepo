...
**Manually resolving a pending desk:** the same pending indicator also shows a **Sync now** link (editors only). It calls `syncDeskNow(deskId)` in `Library.js`, which cancels any scheduled/in-flight auto-backup for that desk (so the two paths never race) and immediately runs `runDeskBackupNow(deskId)` — the same upload-and-index-update logic the 3-second debounce eventually runs, just triggered right away instead of waiting. `deskSyncNowIds` (a `Set` of desk driveIds) drives an in-flight "Syncing…" state on the button while the push is running. This is the only way to force-resolve a dirty desk on demand without waiting for the debounce or running a full library Sync — items/channels already have this via their per-tile Upload button (`onUpload` in `useDriveTileUpload.js`), which re-PATCHes the record and clears its dirty state the same way.
