...
**Desk title / selector** (`DeskSelector`, top-center): always shows the current desk name as a large bold title. When more than one desk exists, a chevron renders and clicking opens a dropdown. The dropdown shows a search input (auto-focused; filters desks by name; clears on close) above a scrollable desk list. Each row has a pencil icon for inline rename (Enter/Escape/blur to save/cancel). A `containerRef` on the root element prevents the blur handler from closing the dropdown when focus moves to the search input. Switching desks calls `onSelectDesk` → `handleSelectDesk` in App, which updates `lastVisitedAt` via `touchItemVisit`. On startup the most recently visited desk is selected automatically.
