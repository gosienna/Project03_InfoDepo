...
- **Sections** (`desk.sections[]`, created via the `/` slash menu "Section" option): rounded-rectangle, theme-tinted grouping containers, each `{id, x, y, width, height, label}`. Rendered behind items/text/connections with `pointerEvents: 'none'` on the body so item interaction is never blocked; only the header strip (drag + click-to-rename label) and the 8 corner/edge resize handles opt back into pointer events. Section-to-item membership is computed live (bounding-box containment against current `layout`/`textItems`) once at the start of a section drag — never persisted — so dragging a section's header moves every item currently inside it together with it. Dragging or placing an item that overlaps/exceeds a section's edge grows that section (grow-only union of its rect with the item's box) to fully contain it; manual resize via the drag handles is the only way to shrink a section. Deleting a selected section (Backspace/Delete) removes only the rectangle, never its contents. Persisted through the same `commitSections`/`setDeskSections` pipeline as `layout`/`textItems`, and included in desk undo/redo history.
- Props: `{ desk, items, channels, desks, onSelectItem, onSelectChannel, onSelectDesk, onUpdateLayout, onUpdateConnections, onUpdateTextItems, onUpdateSections, onDeskModified, onRenameDesk, onSetSharedWith, readOnly, role, onOpenNewNote, onOpenYoutube, onOpenChannel, onOpenFile, onCreateDesk, itemDownloadProgress }`
