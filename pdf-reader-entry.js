...
function DownloadProgress({ name, loaded, total }) {
  ...
  return React.createElement(
    ...
    React.createElement('p', { className: 'text-gray-200 text-sm font-medium text-center max-w-xs leading-snug', style: { wordBreak: 'break-word' } }, name || 'Loading…'),
    React.createElement('div', { className: 'w-72 flex flex-col gap-2' },
      React.createElement('div', { className: 'flex justify-between text-xs' },
        React.createElement('span', { className: 'text-gray-400' }, known ? `${fmtBytes(loaded)} / ${fmtBytes(total)}` : (loaded > 0 ? fmtBytes(loaded) : '')),
        React.createElement('span', { className: 'text-indigo-400 font-semibold tabular-nums' }, known ? `${pct}%` : ''),
      ),
      React.createElement('div', { className: 'h-1.5 rounded-full bg-gray-700 overflow-hidden' },
